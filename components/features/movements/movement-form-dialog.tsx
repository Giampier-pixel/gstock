...
export function MovementFormDialog() {
  ...
  useEffect(() => {
    if (state.ok) { toast.success('Movimiento registrado'); setOpen(false); }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-card/95 backdrop-blur-xl border border-white/60 sm:max-w-md">
        ...
        <form action={formAction} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="product" className="text-xs font-semibold uppercase text-muted-foreground tracking-wider">Producto</Label>
            <Input id="product" name="product" className="bg-white/5 border-white/10 h-10" />
            {state.fieldErrors?.product?.[0] && <p className="text-destructive text-xs">{state.fieldErrors.product[0]}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="quantity" className="text-xs font-semibold uppercase text-muted-foreground tracking-wider">Cantidad</Label>
            <Input id="quantity" name="quantity" type="number" className="bg-white/5 border-white/10 h-10" />
